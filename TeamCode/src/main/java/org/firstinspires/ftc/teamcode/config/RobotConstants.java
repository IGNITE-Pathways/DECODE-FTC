...
public class RobotConstants {
    ...
    // ==================== ALLIANCE COLOR ====================
    /**
     * Alliance color for match configuration
     */
    ...
    public static final double FLYWHEEL_KP = 0.028538;  // Proportional gain
    ...
    public static final double FLYWHEEL_KF = 0.000250;  // Feedforward gain
    ...
    // ==================== GOAL POSITIONS (Field Coordinates) ====================
    // FTC field is 144" x 144" (12ft x 12ft)
    // Origin (0, 0) is at field center
    // Adjust these based on actual goal/basket locations for your game

    // Blue alliance goal position (inches from field center)
    public static final double BLUE_GOAL_X = 70.0;      // Center of field (X)
    public static final double BLUE_GOAL_Y = -70.0;    // Back wall (negative Y)

    // Red alliance goal position (inches from field center)
    ...
    // ==================== DISTANCE-BASED SHOOTING EQUATIONS ====================
    // Uses linear interpolation between tested ranges for smooth transitions

    /**
     * Calculate optimal flywheel RPM based on distance to goal
     * Uses linear interpolation between tested range values
     *
     * @param distanceFeet Distance to goal in feet
     * @return Target flywheel RPM
     */
    public static double calculateFlywheelRPM(double distanceFeet) {
        // Distance-RPM data points from tested ranges (using range midpoints)
        double[] distances = {2.95, 4.055, 5.005, 5.68, 6.505, 7.755, 9.255, 11.0};
        double[] rpms = {2400, 2600, 2700, 2800, 2950, 3150, 3300, 3450};
    ...
    /**
     * Calculate optimal hood position based on distance to goal
     * Uses linear interpolation between tested range values
     *
     * @param distanceFeet Distance to goal in feet
     * @return Hood servo position (0.0 - 1.0)
     */
    public static double calculateHoodPosition(double distanceFeet) {
        // Distance-Hood data points from tested ranges (using range midpoints)
        double[] distances = {2.95, 4.055, 5.005, 5.68, 6.505, 7.755, 9.255, 11.0};
        double[] hoods = {0.55, 0.65, 0.70, 0.70, 0.72, 0.73, 0.74, 0.735};

        // Handle out of range
        if (distanceFeet <= distances[0]) return hoods[0];
        if (distanceFeet >= distances[distances.length - 1]) return hoods[hoods.length - 1];
    ...
    /**
     * Calculate distance from robot position to goal
     *
     ...
     */
    public static double calculateDistanceToGoal(double robotX, double robotY, double goalX, double goalY) {
        double dx = goalX - robotX;
        double dy = goalY - robotY;
        double distanceInches = Math.sqrt(dx * dx + dy * dy);
        return distanceInches / 12.0;  // Convert to feet
    }
}
