...
public class SpindexerOLD extends LinearOpMode {
    ...
    // Soup ladle config (keeping original values to preserve functionality)
    private static final double SOUP_LADLE_RESET_POSITION = 1;
    private static final double SOUP_LADLE_FLICK_POSITION = 0.6;
    ...
    public void runOpMode() {
        // Initialize hardware using HardwareConfig
        ...
        soupLadle = hardwareMap.get(Servo.class, HardwareConfig.BALL_LAUNCHER_SERVO);
        kickerWheel = hardwareMap.get(CRServo.class, HardwareConfig.KICKER_WHEEL);
    ...
    private void displayTelemetry(float hue) {
        ...
        telemetry.addData("⚡ SOUP LADLE", soupLadle.getPosition() < 0.8 ? "FIRING" : "REST");
    ...
    private void kickBall() {
        ...
        soupLadle.setPosition(0.9);
        sleep(100);

        soupLadle.setPosition(0.3);
        sleep(KICK_PULSE_MS);

        // Return soup ladle to rest
        soupLadle.setPosition(SOUP_LADLE_RESET_POSITION);
